...
async def detect_anomalies(readings: list[dict], daily_summaries: list[dict]) -> dict:
    ...
    context += "RULES:\n"
    context += "- Return AT MOST 2 anomalies. Only the most significant ones.\n"
    context += "- Only flag things that deviate >50% from baseline AND would concern a homeowner.\n"
    context += "- Report anomalies for TODAY only — do not list anomalies for each past day individually.\n"
    context += "- These are NOT anomalies (do not report them):\n"
    context += "  * Battery deep discharge to 5% (intentional — optimizer does this during peak)\n"
    context += "  * Low battery state of charge during/after peak hours (same reason)\n"
    context += "  * Home consumption spikes (EV charging adds 7+ kW, this is normal)\n"
    ...
    context += "- ONLY report: equipment malfunctions, grid outages, solar producing 0 on a sunny day, consumption doubling with no explanation\n"
