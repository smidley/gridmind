...
/** Inline component for VPP Peak Events management */
  ...
  const createEvent = async () => {
    ...
    try {
      await apiFetch('/events', {
        method: 'POST',
        body: JSON.stringify({
          name, date, start_time: startTime, end_time: endTime,
          rate_per_kwh: parseFloat(rate) || 0,
        }),
      })
      setShowForm(false)
      setDate('')
  ...
  return (
    <div className="card">
      ...
      <p className="text-xs text-slate-500 mb-4">
        Schedule utility demand response events for premium export rates. The optimizer will automatically dump the battery at maximum rate during the event window.
      </p>
      ...
      {showForm && (
        <div className="mb-4 p-3 rounded-lg bg-violet-500/5 border border-violet-500/20 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="text-sm text-slate-500 hover:text-slate-300 px-3">
              Cancel
            </button>
          </div>
          {error && <p className="text-xs text-red-400">{error}</p>}
            ...
            <div key={evt.id} className="flex items-center justify-between p-3 rounded-lg bg-violet-500/5 border border-violet-500/20">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {evt.date} · {evt.start_time} – {evt.end_time} · <span className="text-violet-400 font-medium">${evt.rate_per_kwh}/kWh</span>
                </p>
              </div>
